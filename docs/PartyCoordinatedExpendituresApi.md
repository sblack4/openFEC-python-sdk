...
> inline_response_default5.InlineResponseDefault5 schedules_schedule_f_get()
...
payee_name = ['payee_name_example'] # [str] |  (optional)
...
sort_nulls_last = False # bool | Toggle that sorts null values last (optional) if omitted the server will use the default value of False
    ...
    # and optional values
    try:
        api_response = api_instance.schedules_schedule_f_get(sort_hide_null=sort_hide_null, max_image_number=max_image_number, image_number=image_number, min_image_number=min_image_number, min_amount=min_amount, candidate_id=candidate_id, payee_name=payee_name, line_number=line_number, committee_id=committee_id, page=page, min_date=min_date, cycle=cycle, sort_null_only=sort_null_only, per_page=per_page, max_amount=max_amount, sort=sort, max_date=max_date, sort_nulls_last=sort_nulls_last)
        pprint(api_response)
    except openfec_sdk.ApiException as e:
        print("Exception when calling PartyCoordinatedExpendituresApi->schedules_schedule_f_get: %s\n" % e)
```
...
[**inline_response_default5.InlineResponseDefault5**](InlineResponseDefault5.md)
...
> inline_response_default5.InlineResponseDefault5 schedules_schedule_f_sub_id_get(sub_id)
...
per_page = 20 # int | The number of results returned per page. Defaults to 20. (optional) if omitted the server will use the default value of 20
    ...
    # and optional values
    try:
        api_response = api_instance.schedules_schedule_f_sub_id_get(sub_id, page=page, per_page=per_page)
        pprint(api_response)
    except openfec_sdk.ApiException as e:
        print("Exception when calling PartyCoordinatedExpendituresApi->schedules_schedule_f_sub_id_get: %s\n" % e)
```
